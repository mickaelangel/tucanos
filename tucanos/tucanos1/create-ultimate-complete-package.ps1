# Script pour créer le package ULTIME COMPLET avec TOUTES les dépendances
...
Write-Host "=== Création du package ULTIME COMPLET pour SUSE 15 ===" -ForegroundColor Green

# Configuration
$SourceDir = "tucanos-main"
$PackageDir = "tucanos-suse15-ultimate-complete"
$RustVersion = "1.90.0"
...
# Télécharger les packages Python nécessaires
Write-Host "=== Téléchargement des packages Python ===" -ForegroundColor Green
try {
    ...
    Move-Item -Path "*.whl" -Destination "$PackageDir/dependencies/python/" -Force -ErrorAction SilentlyContinue
    ...
    Write-Host "⚠ Erreur lors du téléchargement des packages Python: $($_.Exception.Message)" -ForegroundColor Yellow
}
...
try {
    $MetisUrl = "https://github.com/KarypisLab/METIS/archive/refs/heads/master.tar.gz"
    Invoke-WebRequest -Uri $MetisUrl -OutFile "$PackageDir/dependencies/sources/metis-master.tar.gz"
...
# Télécharger CMake (nécessaire pour NLOPT/METIS)
Write-Host "=== Téléchargement de CMake ===" -ForegroundColor Green
try {
    $CmakeUrl = "https://github.com/Kitware/CMake/releases/download/v3.28.1/cmake-3.28.1-linux-x86_64.tar.gz"
    Invoke-WebRequest -Uri $CmakeUrl -OutFile "$PackageDir/dependencies/sources/cmake-3.28.1-linux-x86_64.tar.gz"
    Write-Host "✓ CMake téléchargé" -ForegroundColor Green
} catch {
    Write-Host "⚠ Erreur lors du téléchargement de CMake: $($_.Exception.Message)" -ForegroundColor Yellow
}

# Créer le script d'installation ULTIME COMPLET
Write-Host "=== Création du script d'installation ultime complet ===" -ForegroundColor Green

$UltimateCompleteScript = @"
#!/bin/bash
# Installation ULTIME COMPLETE HORS LIGNE de Tucanos pour SUSE 15
# Inclut TOUTES les dépendances - AUCUNE connexion internet requise

set -e

echo "=== Installation ULTIME COMPLETE HORS LIGNE de Tucanos sur SUSE 15 ==="
...
# Installation de CMake (nécessaire pour NLOPT/METIS)
echo ""
...
if ! command -v cmake &> /dev/null; then
    ...
    if [ -f "dependencies/sources/cmake-3.28.1-linux-x86_64.tar.gz" ]; then
        echo "Installation depuis le package CMake local..."
        cd dependencies/sources
        tar -xzf cmake-3.28.1-linux-x86_64.tar.gz
        sudo cp -r cmake-3.28.1-linux-x86_64/* /usr/local/
        cd ../..
        echo "✓ CMake installé depuis les sources"
    else
        echo "ATTENTION: Aucun package CMake local trouvé"
        echo "CMake doit être installé manuellement:"
        echo "  sudo zypper install cmake"
        echo ""
        read -p "Continuer quand même? (y/N): " -n 1 -r
        echo
    ...
    echo "CMake déjà installé: `$(cmake --version | head -n1)"
fi
...
$UltimateCompleteScript | Out-File -FilePath "$PackageDir/install-ultimate-complete.sh" -Encoding UTF8

# Créer la documentation ultime complète
$UltimateCompleteReadme = @"
# Package ULTIME COMPLET HORS LIGNE Tucanos pour SUSE 15

## 🚫 Installation VRAIMENT hors ligne avec TOUTES les dépendances

Ce package inclut TOUTES les dépendances nécessaires pour une installation complètement hors ligne, y compris Rust, CMake, NLOPT et METIS.

## 📦 Contenu du package

- ✅ **Code source complet** de Tucanos
- ✅ **Rust portable** (rustup-init)
- ✅ **CMake portable** (v3.28.1)
...
- ✅ **Sources METIS** (master) pour compilation locale
...
chmod +x install-ultimate-complete.sh
./install-ultimate-complete.sh
...
- ✅ **Rust, CMake, NLOPT et METIS** inclus
...
- ⚠️ **Taille du package** importante (~300 MB)
- ⚠️ **Temps de compilation** plus long (NLOPT + METIS)
- ⚠️ **Prérequis système** (packages RPM de base)
...
Tu as maintenant un package ULTIME COMPLET qui inclut TOUTES les dépendances pour une installation vraiment hors ligne sur SUSE 15, y compris Rust, CMake, NLOPT et METIS !
"@

$UltimateCompleteReadme | Out-File -FilePath "$PackageDir/README-ULTIMATE-COMPLETE.md" -Encoding UTF8
...
- Sources METIS master (compilation locale)
- Scripts d'installation complets
- Documentation détaillée
...
WORKFLOW:
1. Transférer sur SUSE 15
2. Sur SUSE 15: chmod +x install-ultimate-complete.sh
3. Sur SUSE 15: ./install-ultimate-complete.sh
4. Sur SUSE 15: cd tucanos-install && sudo ./install-system.sh && ./install-python.sh

Documentation: README-ULTIMATE-COMPLETE.md
"@

$VerificationFile | Out-File -FilePath "$PackageDir/VERIFICATION-ULTIMATE-COMPLETE.txt" -Encoding UTF8

Write-Host ""
Write-Host "=== PACKAGE ULTIME COMPLET CRÉÉ ===" -ForegroundColor Green
Write-Host ""
Write-Host "Package ultime complet: $PackageDir" -ForegroundColor Yellow
Write-Host ""
Write-Host "✅ TOUTES les dépendances incluses (Rust, CMake, NLOPT, METIS, Python)" -ForegroundColor Green
Write-Host ""
Write-Host "Instructions ultimes complètes:" -ForegroundColor Yellow
Write-Host "1. Transférer sur SUSE 15" -ForegroundColor White
Write-Host "2. Sur SUSE 15: chmod +x install-ultimate-complete.sh" -ForegroundColor White
Write-Host "3. Sur SUSE 15: ./install-ultimate-complete.sh" -ForegroundColor White
Write-Host "4. Sur SUSE 15: cd tucanos-install && sudo ./install-system.sh && ./install-python.sh" -ForegroundColor White
Write-Host ""
Write-Host "Documentation: $PackageDir/README-ULTIMATE-COMPLETE.md" -ForegroundColor Green
