# Script pour créer le package ULTIME avec NLOPT compilé depuis les sources
# Inclut TOUTES les dépendances possibles
...
Write-Host "=== Création du package ULTIME pour SUSE 15 ===" -ForegroundColor Green

# Configuration
$SourceDir = "tucanos-main"
$PackageDir = "tucanos-suse15-ultimate-package"
$RustVersion = "1.90.0"
...
try {
    # Télécharger NLOPT 2.7.1 (version stable)
...
try {
    # Télécharger METIS 5.1.0 (version stable)
...
# Créer le script d'installation ULTIME
Write-Host "=== Création du script d'installation ultime ===" -ForegroundColor Green

$UltimateInstallScript = @"
#!/bin/bash
# Installation ULTIME HORS LIGNE de Tucanos pour SUSE 15
# Inclut TOUTES les dépendances - AUCUNE connexion internet requise

set -e

echo "=== Installation ULTIME HORS LIGNE de Tucanos sur SUSE 15 ==="
...
$UltimateInstallScript | Out-File -FilePath "$PackageDir/install-ultimate-offline.sh" -Encoding UTF8

# Créer la documentation ultime
$UltimateReadme = @"
# Package ULTIME HORS LIGNE Tucanos pour SUSE 15
...
chmod +x install-ultimate-offline.sh
./install-ultimate-offline.sh
...
Tu as maintenant un package ULTIME qui inclut TOUTES les dépendances pour une installation vraiment hors ligne sur SUSE 15, y compris NLOPT et METIS compilés depuis les sources !
"@

$UltimateReadme | Out-File -FilePath "$PackageDir/README-ULTIMATE.md" -Encoding UTF8
...
WORKFLOW:
1. Transférer sur SUSE 15
2. Sur SUSE 15: chmod +x install-ultimate-offline.sh
3. Sur SUSE 15: ./install-ultimate-offline.sh
4. Sur SUSE 15: cd tucanos-install && sudo ./install-system.sh && ./install-python.sh

Documentation: README-ULTIMATE.md
"@

$VerificationFile | Out-File -FilePath "$PackageDir/VERIFICATION-ULTIMATE.txt" -Encoding UTF8

Write-Host ""
Write-Host "=== PACKAGE ULTIME CRÉÉ ===" -ForegroundColor Green
Write-Host ""
Write-Host "Package ultime: $PackageDir" -ForegroundColor Yellow
Write-Host ""
Write-Host "✅ TOUTES les dépendances incluses (y compris NLOPT et METIS sources)" -ForegroundColor Green
Write-Host ""
Write-Host "Instructions ultimes:" -ForegroundColor Yellow
Write-Host "1. Transférer sur SUSE 15" -ForegroundColor White
Write-Host "2. Sur SUSE 15: chmod +x install-ultimate-offline.sh" -ForegroundColor White
Write-Host "3. Sur SUSE 15: ./install-ultimate-offline.sh" -ForegroundColor White
Write-Host "4. Sur SUSE 15: cd tucanos-install && sudo ./install-system.sh && ./install-python.sh" -ForegroundColor White
Write-Host ""
Write-Host "Documentation: $PackageDir/README-ULTIMATE.md" -ForegroundColor Green
